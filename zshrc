...
configure_prompt() {
    ...
    case "$PROMPT_ALTERNATIVE" in
        twoline)
            PROMPT=$'\033[0;37m[`date  +"%d-%b-%y %T %Z"`]\e[0m\n%F{%(#.white.white)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.red)}%n$prompt_symbol%m%b%F{%(#.white.whit)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.red.white)}]\n└─%B%(#.%F{red}#.%F{red}$)%b%F{reset} '
        ...
        oneline)
            ...
            ;;
        backtrack)
            PROMPT=$'\033[0;37m[`date  +"%d-%b-%y %T %Z"`]\e[0m\n${debian_chroot:+($debian_chroot)}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))}%B%F{red}%n@%m%b%F{reset}:%B%F{white}%~%b%F{reset}%(#.#.$) '
            RPROMPT=
            ;;
    esac
}
...
# Adding Rust bin folder to Path
